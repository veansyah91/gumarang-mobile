...
function DetailErrorState({ message }: { message: string }) {
  return (
    <Card>
      <Text variant="subtitle">Detail penjualan belum bisa dimuat</Text>
      <Text tone="muted">{message}</Text>
    </Card>
  );
}

function ProductCard({ product }: { product: SaleTransactionMemberProduct }) {
  return (
    <Card>
...
export function MemberSaleTransactionDetail() {
  const params = useLocalSearchParams<{ id?: string | string[] }>();
  const id = Array.isArray(params.id) ? params.id[0] : params.id;
  const invoiceId = id ? Number(id) : Number.NaN;

  const query = useQuery({
    queryKey: ['member-sale-transaction-detail', invoiceId],
    queryFn: () => memberApi.getSaleTransactionMember(invoiceId),
    enabled: Number.isFinite(invoiceId) && invoiceId > 0,
    staleTime: 1000 * 60 * 5,
    retry: 1,
  });

  const data: SaleTransactionMemberInvoiceDetail | undefined = query.data;
  ...
  if (!Number.isFinite(invoiceId) || invoiceId <= 0) {
    return (
      <Screen scrollable contentContainerStyle={styles.content}>
        <Card>
          <Text variant="subtitle">ID penjualan tidak valid</Text>
          <Text tone="muted">Parameter detail penjualan tidak ditemukan.</Text>
        </Card>
      </Screen>
    );
  }
