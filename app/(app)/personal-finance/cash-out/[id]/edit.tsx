...
import { useCashOut, useUpdateCashOut } from '@/src/hooks/use-cash-out';
...
import {
  ...
  formStyles,
  getTodayDate,
} from '@/src/components/cash-out-form';

export default function CashOutEditPage() {
  ...
  const { mutateAsync: updateCashOut, isPending: isSubmitting } =
    useUpdateCashOut();
  ...
  useEffect(() => {
    if (transaction) {
      ...
      setCashAccountId(creditEntry?.account_id ?? '');
  ...
  const handleSubmit = async () => {
    ...
    if (validDetails.length === 0) {
      showToast('Tambahkan minimal satu rincian pengeluaran', 'danger');
      return;
    }
    ...
    try {
      ...
      showToast('Kas Keluar berhasil diubah', 'success');
      router.back();
    } catch (err) {
  ...
  if (isDetailLoading) {
    return (
      <>
        <FixedAssetSubHeader title="Edit Kas Keluar" />
