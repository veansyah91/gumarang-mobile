...
export default function CashOutDetailPage() {
  ...
  const { data: transaction, isLoading, isError, refetch, isFetching } =
    useCashOut(transactionId);
  const { mutateAsync: deleteCashOut } =
    useDeleteCashOut();
  ...
  if (isLoading) {
    return (
      <>
        <FixedAssetSubHeader title="Detail Kas Keluar" />
  ...
  const creditEntry = transaction.entries.find(
      ...
      [
        { text: 'Batal', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              await deleteCashOut(transactionId);
              showToast('Cash Out berhasil dihapus', 'success');
              router.back();
            } catch (err) {
    ...
    <>
      ...
      >
        <Card>
          ...
          <DetailRow
            label="Akun Kas"
            value={creditEntry?.account_name ?? '-'}
            icon="cash-outline"
            colors={colors}
          />
          <DetailRow
        ...
        <View style={styles.section}>
          ...
          <View style={styles.detailListCard}>
            {(debitEntries.length > 0 ? debitEntries : []).map(
              (entry, index) => (
                <Pressable
                  key={entry.id}
                  style={({ pressed }) => [
          ...
          <View style={styles.section}>
            ...
            <Card>
              <DetailRow
                ...
                value={transaction.investment.account_name}
                icon="trending-up-outline"
                colors={colors}
              />
              <DetailRow
                label="Jumlah Unit"
                value={String(transaction.investment.unit_quantity)}
                icon="layers-outline"
                colors={colors}
              />
              <DetailRow
                label="Harga Rata-rata"
                value={formatIDR(transaction.investment.unit_cost_avg)}
                icon="calculator-outline"
                colors={colors}
              />
              <DetailRow
                label="Harga Beli"
                value={formatIDR(transaction.investment.unit_price)}
